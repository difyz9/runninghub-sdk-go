...
const defaultPayloadFile = "./examples/image_edit/payload.example.json"

func main() {
	var (
		apiKey       = flag.String("api-key", os.Getenv("RUNNINGHUB_API_KEY"), "RunningHub API key; defaults to RUNNINGHUB_API_KEY")
		modelPath    = flag.String("path", "/openapi/v2/rhart-image-n-pro-official/edit", "image edit model path")
		...
		imageURL     = flag.String("image-url", "", "public image URL to use as imageUrls[0]")
		uploadFile   = flag.String("upload-file", "", "local image file to upload before calling the model")
		outputDir    = flag.String("output-dir", "./examples/image_edit/output", "directory to save result.json and downloaded outputs")
		pollInterval = flag.Duration("poll-interval", 2*time.Second, "poll interval for WaitForTask")
	...
	case *uploadFile != "":
		...
		reqBody["imageUrls"] = []string{up.DownloadURL}
		fmt.Printf("uploaded file URL assigned: imageUrls[0]=%s\n", up.DownloadURL)
	case *imageURL != "":
		reqBody["imageUrls"] = []string{*imageURL}
	}

	if err := validateImageInputs(reqBody); err != nil {
		exitf(err.Error())
	}
...
func validateImageInputs(reqBody map[string]any) error {
	v, ok := reqBody["imageUrls"]
	if !ok {
		return errors.New("missing imageUrls: use -upload-file, -image-url, or set imageUrls in the payload")
	}

	items, ok := v.([]any)
	if ok {
		if len(items) == 0 {
			return errors.New("invalid imageUrls: at least one image URL is required")
		}
		for _, item := range items {
			s, _ := item.(string)
			if strings.TrimSpace(s) != "" && !strings.Contains(s, "replace-with") {
				return nil
			}
		}
		return errors.New("invalid imageUrls: use -upload-file, -image-url, or replace the placeholder in payload.example.json")
	}
	...
	if ok {
		...
		for _, item := range itemsStr {
			if strings.TrimSpace(item) != "" && !strings.Contains(item, "replace-with") {
				return nil
			}
		}
	}

	return errors.New("invalid imageUrls: expected a JSON string array")
}
...
func runTask(ctx context.Context, client *runninghub.Client, modelPath string, reqBody map[string]any, previewOnly bool, pollInterval time.Duration, outputDir string) {
	...
	out, err := client.WaitForTask(ctx, task.TaskID, pollInterval)
	if err != nil {
		exitWithSDKError(err)
	}
