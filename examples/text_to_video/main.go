...
const defaultPayloadFile = "./examples/text_to_video/payload.example.json"
const defaultModelPathValue = "/openapi/v2/seedance-v1.5-pro/text-to-video"

func main() {
	var (
		apiKey       = flag.String("api-key", os.Getenv("RUNNINGHUB_API_KEY"), "RunningHub API key; defaults to RUNNINGHUB_API_KEY")
		modelPath    = flag.String("path", defaultModelPath(), "text-to-video model path; defaults to RUNNINGHUB_TEXT_TO_VIDEO_PATH")
		...
		outputDir    = flag.String("output-dir", "./examples/text_to_video/output", "directory to save result.json and downloaded outputs")
...
func defaultModelPath() string {
	if path := strings.TrimSpace(os.Getenv("RUNNINGHUB_TEXT_TO_VIDEO_PATH")); path != "" {
		return path
	}
